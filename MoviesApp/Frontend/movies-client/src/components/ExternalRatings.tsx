...
const ExternalRatings: React.FC<ExternalRatingsProps> = ({ 
  ...
  useEffect(() => {
    const fetchRatings = async () => {
      try {
        ...
        console.log(`Fetching external ratings for: "${title}" (${year}) - ${isTV ? 'TV Show' : 'Movie'}`);
        
        // Try to search with exact title first
        let externalRatings = await omdbApi.getExternalRatings(title, year, isTV);
        
        // Log the API response for debugging
        console.log('OMDB API response:', externalRatings);
        
        // If no ratings found, try a simplified title (strip special characters)
        if (!externalRatings.imdb && !externalRatings.rottenTomatoes && !externalRatings.metacritic) {
          const simplifiedTitle = title
            .replace(/[^\w\s]/gi, '') // Remove special characters
            .replace(/\s+/g, ' ')     // Remove extra spaces
            .trim();
            
          console.log(`No ratings found, trying simplified title: "${simplifiedTitle}"`);
          
          if (simplifiedTitle !== title) {
            externalRatings = await omdbApi.getExternalRatings(simplifiedTitle, year, isTV);
            console.log('OMDB API response with simplified title:', externalRatings);
          }
        }
        ...
        if (externalRatings.rottenTomatoes) {
          console.log('Setting Rotten Tomatoes rating:', externalRatings.rottenTomatoes.rating);
          setRottenTomatoesRating(externalRatings.rottenTomatoes.rating);
        }
        
        // Set Metacritic data
        if (externalRatings.metacritic) {
          console.log('Setting Metacritic rating:', externalRatings.metacritic.rating);
  ...
  const renderRatings = () => {
    if (isLoading) {
      return <div style={{ color: 'var(--color-text-light)', fontSize: '0.9rem' }}>Loading ratings...</div>;
    }
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--spacing-md)' }}>
        ...
        {rottenTomatoesRating && (
          <div style={{ 
            ...
            <div style={{ 
              backgroundColor: 
                parseInt(formatRottenTomatoesRating(rottenTomatoesRating)) >= 60 ? '#00E33D' : '#FF3822',
              color: 'white',
              fontWeight: 'bold',
              padding: '4px 8px',
              borderRadius: 'var(--radius-sm)',
